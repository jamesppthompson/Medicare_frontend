...
(function(){
	...
	class Libodon {
		...
		connect(server, username){
			...
			.then(reg=>{
				registration = reg
				console.log("return registration>>>", reg)
	...
	async function register_application(server, appname, redirect_url) {
		const endpoint = server+'/api/v1/apps'
		const data = { client_name: "appname" , redirect_uris: "http://xxx.com"};
		...
		}).then(res=>{console.log("response of api>>>",res); res.json()})
		...
		// 	body: data
		// }
		// console.log("register fetch>>>>", fetchInit)
		// fetch("https://mastodon.social/api/v1/apps",fetchInit).then(res=>{console.log("response of api>>>",res); res.json()})
	}
})()